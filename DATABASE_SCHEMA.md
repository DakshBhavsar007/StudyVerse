...
| `banned_at` | DATETIME | NULLABLE | When user was banned |
| `banned_by` | INTEGER | FK → user.id, NULLABLE | Admin who banned the user |
| `referral_code` | VARCHAR(20) | UNIQUE, NULLABLE | Unique referral code |
| `referred_by` | INTEGER | FK → user.id, NULLABLE | Referring user ID |
| **`battle_xp`** | INTEGER | DEFAULT 0 | ⚔️ XP earned exclusively from Byte Battles |
| **`battle_wins`** | INTEGER | DEFAULT 0 | ⚔️ Total Byte Battle wins (all modes) |
| **`battle_losses`** | INTEGER | DEFAULT 0 | ⚔️ Total Byte Battle losses |
...
### **⚔️ Byte Battle Rank System (NEW):**
Battle rank is calculated from `battle_xp` (separate from `total_xp`).

| Battle XP Range | Rank | Icon | Color |
|----------------|------|------|-------|
| 0 – 499 | Recruit | fa-shield-halved | #9CA3AF |
| 500 – 1,999 | Bronze | fa-shield-halved | #CD7F32 |
| 2,000 – 4,999 | Silver | fa-shield-halved | #C0C0C0 |
| 5,000 – 9,999 | Gold | fa-star | #FFD700 |
| 10,000 – 19,999 | Platinum | fa-gem | #E5E4E2 |
| 20,000 – 34,999 | Diamond | fa-gem | #b9f2ff |
| 35,000 – 59,999 | Heroic | fa-crown | #ff4d4d |
| 60,000 – 99,999 | Master | fa-crown | #ff0000 |
| 100,000+ | Grandmaster | fa-dragon | #800080 |

**Battle Stats tracked on `user` table:**
- `battle_xp` — XP earned exclusively from Byte Battle wins/losses
- `battle_wins` — Cumulative battle wins across all game modes
- `battle_losses` — Cumulative battle losses

> ⚠️ **Migration Note:** Run `db.create_all()` or a migration script to add `battle_xp`, `battle_wins`, `battle_losses` columns to the `user` table in existing databases.
...
**Changelog v2.1:** Added `battle_xp`, `battle_wins`, `battle_losses` columns to `user` table for Byte Battle system.
