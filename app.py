...
class User(UserMixin, db.Model):
    ...
    def battle_rank(self):
        return self.battle_rank_info['name']
...
@login_required
def battle():
    # Fetch accepted friends for private invite modal
    ...
    for f in friendships:
        ...
        fr = User.query.get(fid)
        if fr:
            friends.append({'id': fr.id, 'name': fr.first_name or fr.email, 'avatar': fr.get_avatar(40)})
    return render_template('battle.html',
                           battle_rank=current_user.battle_rank_info,
                           friends=friends)
...
def get_team_size(mode):
    """Return players per team for a given mode string."""
    return {'1v1': 1, '2v2': 2, '3v3': 3}.get(mode, 1)

def get_total_slots(mode):
    return get_team_size(mode) * 2
...
def compute_team_scores(room):
    """Compute {A: total, B: total} from individual scores."""
    scores = room.get('scores', {})
    team_scores = {'A': 0, 'B': 0}
    for pid, info in room['players'].items():
        team_scores[info.get('team', 'A')] += scores.get(pid, 0)
    return team_scores

# ── Battle rank: find opponents within 1 tier ──────────────────────────────

def ranks_compatible(rank_a, rank_b):
    """Allow matchmaking within ±1 rank tier."""
    try:
        ia = BATTLE_RANK_ORDER.index(rank_a)
        ib = BATTLE_RANK_ORDER.index(rank_b)
        return abs(ia - ib) <= 1
    except ValueError:
        return True

# ============================================================================
# BATTLE RANK & STATS API
# ============================================================================

@app.route('/api/battle/my_rank')
@login_required
...
@socketio.on('battle_queue_join')
def on_battle_queue_join(data):
    """Player wants auto-matchmaking for a specific mode."""
    if not current_user.is_authenticated:
        return
    mode = data.get('mode', '1v1')  # '1v1', '2v2', '3v3'

    if mode not in matchmaking_queue:
        matchmaking_queue[mode] = []

    # Remove any stale entry for this user
    matchmaking_queue[mode] = [e for e in matchmaking_queue[mode] if e['user_id'] != current_user.id]

    entry = {
        'user_id': current_user.id,
        'sid': request.sid,
        'rank': current_user.battle_rank,
        'name': current_user.first_name or 'Player',
        'joined_at': datetime.utcnow()
    }
    matchmaking_queue[mode].append(entry)

    emit('battle_queue_status', {'status': 'searching', 'mode': mode,
                                  'queue_size': len(matchmaking_queue[mode])})
    print(f"[MM] {current_user.first_name} queued for {mode}. Queue: {len(matchmaking_queue[mode])}")

    # Try to form a match
    _try_form_match(mode)
...
def _try_form_match(mode):
    ...
    # Greedy: Try to find `needed` players within compatible ranks
    # Sort by join time (fairness)
    queue.sort(key=lambda e: e['joined_at'])

    # Find a compatible group: pick first player, then look for compatible ones
    for start_idx in range(len(queue) - needed + 1):
        anchor = queue[start_idx]
        group = [anchor]
        for j in range(start_idx + 1, len(queue)):
            if len(group) >= needed:
                break
            candidate = queue[j]
            if all(ranks_compatible(candidate['rank'], g['rank']) for g in group):
                group.append(candidate)

        if len(group) >= needed:
            selected = group[:needed]
            # Remove selected from queue
            selected_ids = {e['user_id'] for e in selected}
            matchmaking_queue[mode] = [e for e in queue if e['user_id'] not in selected_ids]
            ...
            assign_teams(battles[room_code]['players'], mode)

            # Notify each found player
            for e in selected:
                socketio.emit('battle_match_found', {
                    'room_code': room_code,
                    'mode': mode,
                    'players': [{'name': p['name'], 'team': p['team']} for p in battles[room_code]['players'].values()]
                }, room=e['sid'])

            print(f"[MM] Match formed! Room {room_code}, mode={mode}, players={[e['name'] for e in selected]}")
            return

    # No compatible group found yet – inform all queued players of wait
    for entry in queue:
...
def on_battle_queue_leave(data):
    if not current_user.is_authenticated:
        return
    mode = data.get('mode', '1v1')
    if mode in matchmaking_queue:
        matchmaking_queue[mode] = [e for e in matchmaking_queue[mode] if e['user_id'] != current_user.id]
    emit('battle_queue_status', {'status': 'cancelled', 'mode': mode})


# ============================================================================
# SOCKET.IO – ROOM CREATION & JOINING
# ============================================================================
...
def on_battle_create(data):
    ...
    emit('battle_created', {
        ...
        'mode': mode,
        'visibility': visibility,
        'slots_total': get_total_slots(mode),
        'slots_filled': 1
    })
    print(f"[Battle] Room {room_code} created by {current_user.first_name} | mode={mode} | vis={visibility}")


@socketio.on('battle_invite_friend')
def on_battle_invite_friend(data):
    """Host sends a direct invite to a friend (works for all modes and visibility)."""
    if not current_user.is_authenticated:
        return
    ...
    # Emit to friend's personal room if they're online
    socketio.emit('battle_invite_received', invite_data, room=f'user_{friend_id}')
    # Also store so it's delivered when they open the battle page
    pending_invites.setdefault(friend_id, [])
    ...
    if friend_id not in room.get('invited', []):
...
def on_battle_rejoin_attempt(data):
    ...
    if current_user.id in room['players']:
        ...
        emit('battle_rejoined', {
            ...
            'your_team': room['players'][current_user.id].get('team', 'A'),
...
def on_battle_join_request(data):
    ...
    if room['state'] in ('starting', 'battle'):
        emit('battle_error', {'message': 'Battle already started in this room.'})
        return
    ...
    if current_user.id in room['players']:
        room['players'][current_user.id]['sid'] = request.sid
        join_room(room_code)
        emit('battle_rejoined', {'state': room['state'], 'room_code': room_code, 'mode': room['mode'],
                                  'slots_total': total_slots, 'slots_filled': len(room['players']),
                                  'your_team': room['players'][current_user.id].get('team', 'A'),
                                  'players': {str(k): {'name': v['name'], 'team': v['team']} for k, v in room['players'].items()}})
        return

    # Private room: only invited users can join without host approval
    if room['visibility'] == 'private' and current_user.id not in room.get('invited', []):
        # Still need host approval
        room['pending_join'] = {'user_id': current_user.id, 'name': current_user.first_name or 'Someone', 'sid': request.sid}
        host_sid = room['players'].get(room['host'], {}).get('sid')
        if host_sid:
            socketio.emit('battle_join_request_notify', {
                'name': current_user.first_name or 'Someone',
                'user_id': current_user.id,
                'room_code': room_code
            }, room=host_sid)
        emit('battle_waiting_approval', {'message': 'Waiting for host approval...'})
        return

    # Public room or invited → auto-accept
    _add_player_to_room(room_code, current_user.id, current_user.first_name or 'Player', request.sid)


def _add_player_to_room(room_code, user_id, name, sid):
    """Add a player to the room, assign teams, notify everyone."""
    room = battles[room_code]
    ...
    # Random team assignment every time a player joins
    assign_teams(room['players'], room['mode'])

    total_slots = get_total_slots(room['mode'])
    slots_filled = len(room['players'])

    # Have the joining player join the socket room themselves (reliable)
    # We do NOT use join_room(room_code, sid=sid) — that's unreliable across contexts.
    # Instead we send battle_enter_room to their sid so their client calls battle_entered,
    # which triggers join_room() within their own socket context.
    ...
    # Notify ALL existing players of new roster (host is already in room)
    ...
    # If room is now full, set state to 'ready' and let battle_entered (from the joining client)
    # trigger the start — this avoids background-task/socket-room race conditions
    if slots_filled >= total_slots and room['state'] == 'waiting':
        import random
        ...
        room['state'] = 'ready'   # signals battle_entered to fire start_battle_task
        print(f"[Battle] Room {room_code} is READY. Waiting for all clients to confirm join.")
...
def on_battle_join_response(data):
    ...
    if accepted:
        _add_player_to_room(room_code, pending['user_id'], pending['name'], pending['sid'])
    else:
        socketio.emit('battle_error', {'message': 'Host rejected your request.'}, room=pending['sid'])


@socketio.on('battle_entered')
def on_battle_entered(data):
    """
    Client confirms they are in the waiting room UI.
    This runs in the player's own socket context, so join_room() is reliable here.
    If the room is in 'ready' state (all players joined), this triggers the battle start.
    """
    if not current_user.is_authenticated:
        return
    room_code = data.get('room_code', '').strip().upper()
    ...
    # Join the socket room in OUR OWN context — this is reliable
    join_room(room_code)
    room['players'][current_user.id]['sid'] = request.sid
    room['players'][current_user.id]['confirmed'] = True

    print(f"[Battle] {current_user.first_name} confirmed entry to {room_code} (state={room['state']})")

    # Count how many players have confirmed
    confirmed = sum(1 for p in room['players'].values() if p.get('confirmed'))
    total_slots = get_total_slots(room['mode'])

    # If room is ready and this player is the LAST to confirm → start battle now
    if room['state'] == 'ready' and confirmed >= total_slots:
        room['state'] = 'starting'
        socketio.emit('battle_chat_message', {
            'sender': 'ByteBot',
            'message': (
                f"🎮 All {total_slots} players connected!\n"
                f"⚡ Difficulty: **{room['config']['difficulty']}** | Language: **{room['config']['language']}**\n"
                "🚀 Generating problem and starting battle..."
            ),
            'type': 'system'
        }, room=room_code)
        # Start immediately in background (problem generation may take a moment)
        socketio.start_background_task(start_battle_task, room_code)
        return

    # Otherwise just send them their team status
    team_a = [v['name'] for v in room['players'].values() if v.get('team') == 'A']
    team_b = [v['name'] for v in room['players'].values() if v.get('team') == 'B']
    emit('battle_room_status', {
        'room_code': room_code,
        'mode': room['mode'],
        'state': room['state'],
        'team_a': team_a,
...
# ============================================================================
# SOCKET.IO – BATTLE CHAT & GAME FLOW
# ============================================================================
...
def on_battle_chat_send(data):
    ...
    # System commands
    if message.lower() == '/start' and current_user.id == room['host']:
        total_slots = get_total_slots(room['mode'])
        if room['state'] in ('battle', 'starting'):
            emit('battle_chat_message', {'sender': 'ByteBot', 'message': '⚠️ Battle already in progress!', 'type': 'system'})
            return
        if len(room['players']) < total_slots:
            emit('battle_chat_message', {
                'sender': 'ByteBot',
                'message': f"⏳ Need {total_slots - len(room['players'])} more player(s) before starting.",
                'type': 'system'
            })
            return
        room['state'] = 'starting'
    ...
    # Check if both teams have confirmed settings via chat
    if room['state'] == 'waiting' and message.lower().startswith('/config'):
        # /config easy python
        parts = message.split()
        if len(parts) >= 3 and current_user.id == room['host']:
            room['config']['difficulty'] = parts[1].capitalize()
            room['config']['language']   = parts[2].capitalize()
            emit('battle_chat_message', {
                'sender': 'ByteBot',
                'message': f"✅ Config set: {room['config']['difficulty']} | {room['config']['language']}. Type /start to begin!",
                'type': 'system'
            }, room=room_code)
        return

    # Setup flow: host configures difficulty & language via chat (legacy 1v1 style)
    if room['state'] == 'setup' or (room['state'] == 'waiting' and room['config']['difficulty'] is None):
        if current_user.id == room['host']:
            config = room['config']
            diff_keywords = {'easy': 'Easy', 'medium': 'Medium', 'hard': 'Hard'}
            lang_keywords = {'python': 'Python', 'javascript': 'JavaScript', 'java': 'Java', 'cpp': 'C++', 'c++': 'C++'}

            for kw, val in diff_keywords.items():
                if kw in message.lower():
                    config['difficulty'] = val

            for kw, val in lang_keywords.items():
                if kw in message.lower():
                    config['language'] = val
...
def start_battle_task(room_code):
    """Generate problem and broadcast battle_started to all players in the room."""
    room = battles.get(room_code)
    if not room:
        return
    if room['state'] == 'battle':
        return  # Already started (double-call guard)
    ...
    print(f"[Battle] {room_code} STARTED — {config['difficulty']} {config['language']}")
...
def on_battle_submit(data):
    ...
    socketio.emit('battle_notification', {
        'message': f"🔔 {player_name} has submitted their solution."
    }, room=room_code)
    socketio.emit('battle_chat_message', {
        'sender': 'ByteBot',
        'message': f"🔔 {player_name} submitted. ({len(room['submissions'])}/{len(room['players'])} submitted)",
        'type': 'system'
    }, room=room_code)

    # Judge when all players have submitted
    if len(room['submissions']) >= len(room['players']):
        socketio.emit('battle_state_change', {'state': 'judging'}, room=room_code)
        socketio.start_background_task(judge_battle, room_code)


def generate_battle_problem(difficulty, language):
    prompt = (
        f"Generate a {difficulty} {language} coding problem for a competitive battle. "
        "Reply ONLY with a JSON object with fields: "
        "\"title\", \"description\", \"examples\" (list of {{\"input\":...,\"output\":...}}), \"constraints\" (list of strings). "
        "No extra text."
    )
    try:
        ...
        return {
            ...
            "description": "Write a function that returns the sum of two integers a and b.",
...
def judge_battle(room_code):
    """AI referee judges the battle and awards XP."""
    try:
        room = battles.get(room_code)
        if not room:
            return

        problem_desc = room['problem'].get('description', 'N/A') if room['problem'] else 'N/A'
        name_to_id   = {v['name']: k for k, v in room['players'].items()}
        submissions  = {room['players'][pid]['name']: code for pid, code in room['submissions'].items()}

        prompt = (
            f"You are the referee of a coding battle. Problem: {problem_desc}\n"
            f"Submissions: {json.dumps(submissions)}\n"
            "Judge each submission for correctness and efficiency. "
            "Reply ONLY with a JSON object: "
            "{\"winner\": \"<player_name or team_name or 'Draw'>\", "
            "\"scores\": {\"<name>\": <0-100>}, "
            "\"feedback\": {\"<name>\": \"<brief feedback>\"}, "
        ...
        # Update individual scores from AI
        ...
        # ── Determine winner & award Battle XP ──────────────────────────
        difficulty      = room['config'].get('difficulty', 'Easy')
        xp_1v1_map      = {'Easy': 100, 'Medium': 500, 'Hard': 1000}
        base_xp_1v1     = xp_1v1_map.get(difficulty, 100)

        # Team XP overrides: fixed per mode
        team_xp_win  = {'1v1': base_xp_1v1, '2v2': 800,  '3v3': 1500}
        team_xp_draw = {'1v1': base_xp_1v1 // 2, '2v2': 400, '3v3': 750}

        win_xp  = team_xp_win.get(mode,  base_xp_1v1)
        draw_xp = team_xp_draw.get(mode, base_xp_1v1 // 2)

        winner_name  = result.get('winner', 'Draw')
        winner_id    = name_to_id.get(winner_name)
        xp_awarded   = {}

        if mode == '1v1':
            if winner_id:
                _award_battle_xp(winner_id, win_xp, won=True)
                xp_awarded[winner_name] = win_xp
                # Loser gets nothing (or small consolation)
            ...
            team_scores = compute_team_scores(room)
            ...
            result['team_scores'] = team_scores
            result['winning_team'] = winning_team or 'Draw'

            for pid, pinfo in room['players'].items():
                pname = pinfo['name']
                if winning_team and pinfo.get('team') == winning_team:
...
def _award_battle_xp(user_id, amount, won: bool):
    ...
    with app.app_context():
        ...
        if not user:
            return
        if amount > 0:
            user.battle_xp = (user.battle_xp or 0) + amount
            # Also add to study XP via GamificationService
...
# ============================================================================
# SOCKET.IO – REMATCH, HEARTBEAT, LEAVE
# ============================================================================
...
def on_battle_rematch_vote(data):
    ...
    socketio.emit('battle_chat_message', {
        'sender': 'ByteBot',
        'message': f"{player_name} voted: {vote.upper()}",
        'type': 'system'
    }, room=room_code)
    ...
    if vote == 'no':
        ...
        socketio.emit('battle_rematch_declined', {}, room=room_code)
        eventlet.sleep(3)
...
# ── Personal room join on connect (for private invites) ───────────────────
...
def on_join_personal_room(data):
    ...
    if current_user.is_authenticated:
        ...
        # Deliver any invites stored while user was on another page
        for invite in pending_invites.pop(current_user.id, []):
            rc = invite.get('room_code')
            if rc in battles:
                r = battles[rc]
                if r['state'] == 'waiting' and len(r['players']) < get_total_slots(r['mode']):
